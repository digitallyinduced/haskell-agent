...
, JuicyPixels, lib, network-uri, safe-exceptions, stm, text, time
, transformers, unix, uuid, websockets, wuss
}:
mkDerivation {
  ...
  testHaskellDepends = [
    ...
    filelock filepath hspec http-client http-client-tls http-types
    JuicyPixels safe-exceptions stm text time unix websockets wuss
  ];
