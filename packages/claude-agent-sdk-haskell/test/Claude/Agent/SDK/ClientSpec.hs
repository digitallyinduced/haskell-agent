...
spec = describe "ClaudeSDKClient subprocess transport" do
    ...
    it "accepts multi-megabyte structured output records by default" do
        ...
        let recordTextBytes = 2 * 1024 * 1024
        withFakeClaude
            (largeRecordScript recordTextBytes)
            \directory executable -> do
...
-- @textBytes@ characters long, followed by a success result. The payload is
-- generated by the script rather than embedded in it, so the record arrives
-- across many pipe reads like a real oversized tool result.
largeRecordScript :: Int -> String
largeRecordScript textBytes =
    Text.unpack $
        Text.unlines
            [ "#!/bin/sh"
            , "IFS= read -r _query"
