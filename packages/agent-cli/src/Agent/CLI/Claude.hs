...
exitPlanMode :: PlanModeEnv -> IO ClaudeCodePermissionResult
exitPlanMode plan = do
    markdown <- readPlanMarkdown plan
    plan.planHooks.planDecideExit markdown >>= \case
        ...
        PlanRequestChanges feedback ->
            pure $
                deny
                    ("The user requested plan changes: " <> feedback)
        PlanCancel -> do
            deactivatePlanMode plan
