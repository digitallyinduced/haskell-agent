...
connectMailImapAccount rawEmail rawLabel settings password =
    case (normalizeMailEmail rawEmail, validateMailImapSettings settings,
          validatePassword password) of
        (Left err, _, _) -> pure (Left err)
        (_, Left err, _) -> pure (Left err)
        (_, _, Left err) -> pure (Left err)
        (Right email, Right (), Right ()) ->
                ...
                Right () -> do
                    loadMailAccounts >>= \case
                        ...
                        Right accounts -> do
                            ...
                            let createdAt = maybe now (.mailAccountCreatedAt)
                                    (find (sameMailbox email) accounts)
                                account = MailAccount
