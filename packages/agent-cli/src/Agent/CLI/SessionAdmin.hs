...
runListSessions outputFormat = do
    ...
    withStoreForHome home \store -> do
        ...
        (sessions, warnings) <- listSessions pool root
...
containsInputImage raw =
    maybe False containsInputImageValue
        (Aeson.decodeStrict' (rawJsonBytes raw))

containsInputImageValue :: Aeson.Value -> Bool
containsInputImageValue = \case
    Aeson.Object object ->
        KeyMap.lookup "type" object == Just (Aeson.String "input_image")
            || any containsInputImageValue object
...
renderToolValue raw =
    case Aeson.decodeStrict' (rawJsonBytes raw) of
        ...
        _ -> TextEncoding.decodeUtf8 (rawJsonBytes raw)
