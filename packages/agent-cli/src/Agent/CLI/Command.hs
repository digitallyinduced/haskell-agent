...
import Agent.CLI.Command.Menu (buildMenuCompletionData, commandMenu, fuzzyMatch)
...
mkSlashCatalog fastMode dialect toolNames skills modelIds =
    let tools =
        ...
        (completionIndex, completionCommands) = buildMenuCompletionData commands skills
    in SlashCatalog
        ...
        , slashCatalogCompletionCommands = completionCommands
...
slashCatalogWithSkills skills catalog =
    let (completionIndex, completionCommands) =
            buildMenuCompletionData catalog.slashCatalogCommands skills
    in catalog
        { slashCatalogSkills = skills
        , slashCatalogSkillByName = indexSkillCommands skills
        , slashCatalogCompletionIndex = completionIndex
...
completeSlashNames catalog word =
    ...
    in map Text.unpack $
        completeCommandNames catalog.slashCatalogCompletionIndex needle
