    ...
    pure
        ...
        , BenchmarkTask
            ...
            , taskForcedHaskellGuidance = Text.unlines
                [ "For this semantic triage task, after reading the eight reports, make exactly one isolated callLLMText request per report."
                ...
                , "Ask each nested response to extract exactly `ID|STATUS|IMPACT|AFFECTED|UNAUTHORIZED|CAUSE`, use the Text returned by callLLMText directly, and parse the records in Haskell."
                , "Compute severity deterministically in Haskell from IMPACT, AFFECTED, and UNAUTHORIZED; do not ask the nested model to classify P1/P2/P3."
                , "Do not put import declarations inside the do block and do not render or manually traverse the raw Response value."
...
incidentTriagePrompt = Text.unlines
    ...
    , "Each isolated extractor must return exactly `ID|STATUS|IMPACT|AFFECTED|UNAUTHORIZED|CAUSE`, for example `INC-241|open|complete|18400|no|certificate-expiry`."
    , "IMPACT must be exactly complete, partial, or other. UNAUTHORIZED must be exactly yes or no."
    , "The isolated model must extract current facts only; it must not classify P1/P2/P3."
    , "For callLLMText, use an explicitly typed request binding. Its input must be `ResponseInputItems [MessageItem ResponseMessage{...}]` containing an `InputTextPart`; do not use `ResponseInputText`, because the OpenAI Codex transport requires an item list."
    , "Use the Text returned by callLLMText directly. Do not call raw callLLM, render a Response with show, or manually traverse Response.output."
    ...
    , "- Compute severity in Haskell: P1 if UNAUTHORIZED=yes, or IMPACT=complete and AFFECTED>=10000; otherwise P2 if IMPACT=complete, or IMPACT=partial and AFFECTED>=2000; otherwise P3."
    ...
    , "- Build the result with `object` and `(.=)`, then serialize it with `encodeJsonText`. Do not manually concatenate JSON and do not use `show` for JSON."
    , ""
    , "Return exactly one final line and no other prose:"
    , "BENCH_RESULT {\"openP1\":[...],\"openP2\":[...],\"openP3\":[...],\"openAffected\":N,\"resolvedCount\":N,\"duplicateCauses\":[...]}"
    ]
