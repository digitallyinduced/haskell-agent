...
spec = do
    describe "modelsForProvider" do
        ...
        it "assigns the owned-tool dialect to every Claude Code model" do
            map (.modelDialect) (modelsForProvider ClaudeCodeProvider)
    ...
    describe "resolveModelOptionDialect" do
        ...
        it "keeps direct providers on their native dialect" do
            ...
            claude <-
                resolveModelOptionDialect ModelOption
                    { modelProvider = ClaudeCodeProvider
                    , modelId = "sonnet"
                    , modelTransportId = "sonnet"
                    , modelDialect = CodexDialect
                    , modelLabel = Nothing
                    }
            claude.modelDialect `shouldBe` ClaudeCodeDialect
