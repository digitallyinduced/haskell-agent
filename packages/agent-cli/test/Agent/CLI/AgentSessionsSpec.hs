...
spec = describe "Agent.CLI.AgentSessions" do
    ...
    it "does not hold the process manager lock while a foreground turn runs" $
        withTempDir "agent-session-runtime-" \root -> do
                    ...
                    \_ -> do
                        ...
                        readMVar finished >>= \case
                            Left err -> expectationFailure
                                ("foreground launcher failed: " <> show err)
                            Right result ->
                                result `shouldBe`
                                    Right
                                        ("completed session "
    ...
    it "keeps the child-owned lock when a foreground waiter is canceled" $
        withTempDir "agent-session-runtime-" \root -> do
            let startedPath = toFilePath root FilePath.</> "started"
                releasePath = toFilePath root FilePath.</> "release"
            script <- writeFakeAgentBody root $
                ...
                bracket
                    ...
                    (\launcher -> do
                        ...
                        waitForSessionStopped
                            manager handle.sessionMeta.metaId
                    ...
                    \launcher -> do
                        ...
                        readMVar finished >>= \case
                            ...
                            Right result -> expectationFailure
                                ("foreground launcher was not canceled: "
                                    <> show result)
                        Directory.doesDirectoryExist lockPath
                            `shouldReturn` True
                        -- The in-process reservation must remain even if the
                        -- cross-process directory disappears unexpectedly.
...
waitForSessionStopped :: SessionProcessManager -> Text.Text -> IO ()
waitForSessionStopped manager sessionId = go (50 :: Int)
  where
    go 0 = expectationFailure
        ("timed out waiting for session " <> Text.unpack sessionId)
    go attempts =
        sessionProcessStatus manager sessionId >>= \case
            "running" -> threadDelay 20000 >> go (attempts - 1)
            _ -> pure ()
