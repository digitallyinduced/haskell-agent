...
--
-- Gateway values identify only a parent-owned loopback proxy and its
-- short-lived local capability. They must never contain the organization
-- gateway URL or bearer.
...
anthropicGatewayBaseUrl base =
    Text.dropWhileEnd (== '/') (Text.strip base)
