module Agent.ComputerUse.Linux
    ( LinuxSessionType(..)
    , detectLinuxSessionType
    , newLinuxBackend
    ) where

import Agent.ComputerUse.Backend (ComputerBackend(..))
import Agent.ComputerUse.Linux.Logind
    ( LogindGuard(..)
    , LogindSessionTarget(..)
    , newLogindGuard
    )
import Agent.ComputerUse.Linux.Portal (newPortalBackend)
import Agent.ComputerUse.Linux.X11 (newX11Backend)
