...
customQueryStatement rowCap query =
    Statement.unpreparable sql Encoders.noParams customQueryResultDecoder
  where
    overflowCap
    ...
    sql =
        "select coalesce(string_agg("
            ...
            <> "limit " <> capPlusOne
            <> ") as q"
-- | Render arbitrary query rows as one JSON array for trusted structured
-- clients such as the native data browser. The query still runs through the
-- same scope isolation, read-only transaction, timeout, and size limits as
-- the human-readable query path.
customJsonQueryStatement :: Int64 -> Text -> Statement () CustomQueryResult
customJsonQueryStatement rowCap query =
    Statement.unpreparable sql Encoders.noParams customQueryResultDecoder
  where
    overflowCap
        | rowCap == maxBound = rowCap
    ...
    capPlusOne = Text.pack (show overflowCap)
    sql =
        "select coalesce(jsonb_agg(q._ha_row order by q._ha_row_number) "
            <> "filter (where q._ha_row_number <= " <> cap <> "), "
            <> "'[]'::jsonb)::text, "
            <> "coalesce(max(q._ha_row_number), 0) > " <> cap
            <> " from ("
...
customQueryResultDecoder :: Decoders.Result CustomQueryResult
customQueryResultDecoder = Decoders.singleRow $
    CustomQueryResult
