module Agent.TUI.SyntaxSpec (spec) where
...
spec :: Spec
spec = describe "fullscreen syntax highlighting" do
...
sourceSyntaxDirectory = do
    let packageLocal = "data/syntax"
        repositoryLocal = "packages/agent-tui/data/syntax"
    packageLocalExists <- doesDirectoryExist packageLocal
    pure
        (if packageLocalExists
            then packageLocal
