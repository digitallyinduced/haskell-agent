-- Rejected experiment: see TerminalText.md. Not used in production.
-- | Terminal-cell width classification shared by line editors and renderers.
module TerminalTextCandidate
    ( charCellWidth
...
displayTerminalText input
    | Text.all (\c -> (c >= ' ' && c <= '~') || c == '\n') input = input
    | otherwise = displayTerminalTextWithGraphemes input

displayTerminalTextWithGraphemes :: Text -> Text
displayTerminalTextWithGraphemes =
