...
haskellProgramDescription =
    ...
    \Use `callLLMText request :: IO Text` for the assistant's output text, or `callLLMJson :: FromJSON value => ResponseCreateParams -> IO value` when the assistant returns JSON. Use `callLLM request :: IO Response` only when you need the complete raw Responses API value. `responseOutputTexts`, `responseOutputText`, `decodeJsonText`, and `encodeJsonText :: ToJSON value => value -> Text` are also preimported helpers. Use encodeJsonText rather than hand-building final JSON. `ResponseCreateParams`, `Response`, their related canonical types, and `defaultResponseCreateParams` are preimported from `Agent.Responses.Types`. Provider transports may enforce required wire settings; the OpenAI Codex transport sends streaming, non-stored responses.\n\
    ...
    \callLLM returns the complete lossless response, including output items, usage, response ids, unknown fields, and raw tool calls; it does not run an automatic nested agent loop. Prefer the text/JSON helpers so response traversal and decoding failures are explicit. Identical successful callLLM requests are memoized across repaired program attempts during the current parent turn; failed requests are retried normally.\n\
