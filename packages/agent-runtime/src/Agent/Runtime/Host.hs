-- | Host capabilities and callbacks, independent of terminal presentation.
module Agent.Runtime.Host
    ( ActiveHttpAuth(..)
    , AgentProcessRuntime(..)
    , AgentRunMode(..)
...
import Agent.Runtime.Session.Threads ( SessionThreadManager )
import Agent.Runtime.AgentSnapshot ( AgentSnapshot )
import Agent.Runtime.Project ( ProjectSettings )
...
data NativeRunHooks = NativeRunHooks
    ...
    , nativeRegisterAgentSnapshot :: !(IO [AgentSnapshot] -> IO ())
