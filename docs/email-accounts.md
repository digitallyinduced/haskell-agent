# Connected read-only email accounts

Haskell Agent can keep connection metadata for Gmail, Microsoft 365/Outlook,
and custom IMAP mailboxes. Email access is read-only. The runtime does not
request send, draft, delete, move, or label-write permissions.

## Security model

- Gmail authorization requests `gmail.readonly`; Microsoft requests
  `Mail.Read` and `User.Read`. Both browser flows use a random PKCE verifier,
  state validation, a short-lived loopback listener bound to `127.0.0.1`, and
  an opaque flow ID.
...
Gmail and Microsoft support all five tools. Custom IMAP accounts support
...
mailbox listing, structured search, and bounded message reads. Gmail and Graph
attachments retain their provider attachment IDs. For custom IMAP, the
runtime checks `RFC822.SIZE` before reading the message literal, parses that
bounded MIME message locally, and returns only the requested decoded
attachment part. Oversized messages or parts fail before a file is written.
